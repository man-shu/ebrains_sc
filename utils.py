...
def mrconvert_nifti2mif(nifti, mif, tmp_dir):
    ...
    if check_outputs_exist([os.path.join(tmp_dir, mif)]):
        print("Outputs already exist, skipping mrconvert_nifti2mif.")
        return
    print(cmd2)
    os.system(cmd2)
...
def tcktransform_tract2mni(tck, mni_tck, tmp_dir):
    ...
    if check_outputs_exist([mni_tck]):
        print("Outputs already exist, skipping tcktransform_tract2mni.")
        return
    print(cmd8)
    os.system(cmd8)
...
def atlas2dwi(dwi_b0, mni_nifti, atlas_nifti, atlas_in_dwi_space):
    ...
    if check_outputs_exist([atlas_in_dwi_space]):
